...
if command -v claude &>/dev/null; then
  ...
  claude_current=$(echo "$claude_output" | grep "Current version" | awk '{print $NF}')
  if echo "$claude_output" | grep -qi "up to date"; then
    echo -e "  Version: ${claude_current}"
    echo -e "${GREEN}  Claude Code is up to date.${RESET}"
    claude_status="up to date"
    add_summary "Claude Code" "$claude_current" "" "Up to date" ""
  else
    claude_latest=$(echo "$claude_output" | grep -i "available\|latest\|updated" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | tail -1)
    echo -e "  Current: ${YELLOW}${claude_current}${RESET}"
    echo -e "  Latest:  ${GREEN}${claude_latest:-check manually}${RESET}"
    echo ""
    echo -e "${YELLOW}Updating Claude Code...${RESET}"
    claude update 2>&1 || true
    claude_status="updated (${claude_current} → ${claude_latest:-?})"
    add_summary "Claude Code" "$claude_current" "${claude_latest:-?}" "Updated" ""
  fi
fi
...
if brew list --cask 2>/dev/null | grep -q "^warp$"; then
  ...
  if [[ -n "$warp_outdated" ]]; then
    ...
    warp_short_old=$(echo "$warp_installed" | sed -E 's/^0\.//; s/\.[0-9]{2}\.[0-9]{2}\.stable.*//')
    warp_short_new=$(echo "${warp_latest:-?}" | sed -E 's/^0\.//; s/\.[0-9]{2}\.[0-9]{2}\.stable.*//')
    ...
    warp_short=$(echo "$warp_installed" | sed -E 's/^0\.//; s/\.[0-9]{2}\.[0-9]{2}\.stable.*//')
...
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo ""
printf "  ${BOLD}%-18s %-16s %-16s %-14s %-35s${RESET}\n" "CLI" "Previous Ver" "Updated Ver" "Status" "Notes"
...
for i in "${!summary_names[@]}"; do
  ...
  if [[ "$status" == "Updated" ]]; then
    printf "  %-18s ${YELLOW}%-16s${RESET} ${GREEN}%-16s${RESET} ${GREEN}%-14s${RESET} %-35s\n" "$name" "$old_ver" "$new_ver" "$status" "$notes"
  elif [[ "$status" == "Up to date" ]]; then
    printf "  %-18s %-16s %-16s ${GREEN}%-14s${RESET} %-35s\n" "$name" "$old_ver" "—" "$status" "$notes"
  else
    printf "  %-18s %-16s %-16s ${YELLOW}%-14s${RESET} %-35s\n" "$name" "$old_ver" "$new_ver" "$status" "$notes"
  fi
done
echo ""
